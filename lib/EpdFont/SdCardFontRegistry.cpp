...
bool SdCardFontRegistry::parseFilename(const char* filename, uint8_t& size, uint8_t& style) {
  // Bundled naming: <name>_<size>.cpfont (e.g. Bookerly-SD_14.cpfont)
  ...
  // Modern .cpfont files bundle every style (regular/bold/italic/bold-italic) into
...
void SdCardFontRegistry::scanDirectory(const char* dirPath, SdCardFontFamilyInfo& family) {
  ...
  while (true) {
    ...
    // The bundle-everything design means parseFilename always returns style=0, so
